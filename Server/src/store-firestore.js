...
const tsObj = (ts) => ts && typeof ts.seconds === 'number'
...
// --- Requests ---
export async function createRequest({ item, platform, latitude, longitude, expiresInMinutes }, user) {
...
export async function listMine(uid) {
  ...
  const items = snap.docs.map(doc => {
    ...
    return {
      ...
      lat: d.lat,
      lng: d.lng,
      deleteAt: d.deleteAt ? { _seconds: d.deleteAt.seconds, _nanoseconds: d.deleteAt.nanoseconds ?? 0 } : null,
      uid: d.uid,
      displayName: d.displayName,
...
export async function ensureMembership(requestId, uid) {
  ...
  await mref.set({ joined: true, joinedAt: Timestamp.fromMillis(Date.now()) }, { merge: true });
  return true;
}
...
// --- Nearby & Messages unchanged (omitted for brevity) ...
...
export async function listMessages(requestId) {
  const q = db.collection('requests').doc(requestId).collection('messages').orderBy('createdAt', 'asc');
  const snap = await q.get();
  return snap.docs.map(serializeMessageDoc);
}

export async function addMessage(requestId, { uid, displayName, text }) {
  const mref = db.collection('requests').doc(requestId).collection('messages').doc(nanoid(12));
  const msg = {
    uid: uid || 'anon',
...
export async function cleanupExpiredRequests(limit = 50) {
  const nowTs = Timestamp.fromMillis(Date.now());
  const expiredQ = db.collection('requests')
    .where('deleteAt', '<=', nowTs)
    .limit(limit);
